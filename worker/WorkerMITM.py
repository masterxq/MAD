...
class WorkerMITM(MITMBase):
    ...
    def __update_injection_settings(self):
        ...
        if ids_iv:
            ...
            if encounter_ids:
                logger.debug("Found %d new encounter_ids", len(encounter_ids))
                for encounter_id, disappear in encounter_ids.items():
                    logger.debug("id: %d, despawn: %d", encounter_id, disappear)
            ...
            for key, value in self._encounter_ids.items():
                if (value < max_age):
                    remove.append(key)
                    logger.debug("removing encounterid: %d mon despawned", key)

            for key in remove:
                del self._encounter_ids[key]

            logger.debug("Encounter list len: %d", len(self._encounter_ids))
